...
positions = {
    ...
    {{935.51, 46.65, 81.1, 0}, {1089.81, 205.96, -49.0},{255,0,0}, "Casino Diamond"}, -- Outside the Sheriff's Station
    ...
    while true do
        ...
        for _,location in ipairs(positions) do
			...
			if CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc1.x, loc1.y, loc1.z, 2) then 
				DrawText3Ds(loc1.x, loc1.y, loc1.z, '[~g~E~s~] Enter Casino')
			...
			elseif CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc2.x, loc2.y, loc2.z, 2) then
				DrawText3Ds(loc2.x, loc2.y, loc2.z, '[~g~E~s~] Exit Casino')
                if IsControlJustReleased(1, key_to_teleport) then
					DoScreenFadeOut(500)
    				Wait(500)
...
function setCasinoBlip()
    ...
    created_ped2 = CreatePed(0, modelHash , 1109.34, 228.68,  -50.64, true)
    FreezeEntityPosition(created_ped2, true)
